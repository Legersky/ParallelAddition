...
def _(frame_label=text_control('<h3>Load inputs: </h3>', label=''),
 ...
 #  try:
        ...
        for a in range(0,2):
            for b in range(0,2):
                lattice.append(a+b*alg.getBase())
                lattice.append(-a-b*alg.getBase())
                lattice.append(a-b*alg.getBase())
                lattice.append(-a+b*alg.getBase())
        print "tu"
        def polygon_shifted2(points,shift=0, enlargement=1.2, color='green'):
            vertices=[]
            global alg
            for point in points:
                    vertices.append(enlargement*alg.getCoordinates(point) +alg.getCoordinates(shift))
        ...
        for point in lattice:
            tiles+=polygon_shifted2(alg.getAlphabet(),shift=alg.getBase()*point)
        show(alg.plot(lattice)+tiles)
        show(alg.plot(alg.getAlphabet()))
        alg_update=False
